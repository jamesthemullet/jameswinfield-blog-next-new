...
- [x] Add `sitemap.xml` and `robots.txt` — neither exists anywhere in the repo (no static file, no `pages/sitemap.xml.ts`, no `next-sitemap` config), so search engines have no crawl directives or sitemap discovery path (found: 2026-09-01) (resolved: 2026-09-17, PR #TBD)
