...
- [x] Add JSON-LD structured data (`Article`/`BlogPosting`) to `pages/posts/[slug].tsx` — published date and author are currently visual-only with no `application/ld+json` block, so rich results aren't available to search engines (found: 2026-09-01) (resolved: 2026-09-19, PR TBD)
